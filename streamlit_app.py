...
# --- THE VAULT (Retrieving Keys from Secrets) ---
try:
    ...
    st.error(f"CRITICAL: Secrets Missing. Check Streamlit Dashboard. Error: {e}")
    st.stop()
...
def get_canvas_tasks():
    ...
    try:
        ...
        return "\n".join(tasks) if tasks else "No upcoming Canvas tasks found."
...
def add_calendar_event(summary, start_time_str):
    """Adds an event. Expects ISO Format (YYYY-MM-DDTHH:MM:SS)."""
    try:
        creds = get_google_creds()
        service = build('calendar', 'v3', credentials=creds)
        ...
        return f"SUCCESS: Scheduled '{summary}' for {start_time_str}."
...
PROTOCOL:
- Always check Context Data before answering.
- If asking to schedule, use the exact format YYYY-MM-DDTHH:MM:SS.
...
# --- THE INTERFACE ---
st.markdown("<h1 style='text-align: center; color: #a855f7; font-family: monospace; letter-spacing: 4px;'>O.M.N.I.</h1>", unsafe_allow_html=True)
...
if prompt := st.chat_input("Command the System..."):
    ...
    with st.chat_message("assistant"):
        ...
        with st.status("Initializing Neural Handshake...", expanded=False) as status:
            if any(x in p_lower for x in ['due', 'school', 'assignment', 'homework']):
